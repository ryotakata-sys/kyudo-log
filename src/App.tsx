...
/**
 * 弓道「矢所ログ」V7.2 (Judgment Fix + Performance)
 * - 的中判定を修正：3つの的すべてで正しく「的中」が出るよう改善
 * - iPad操作感：transition削除とtranslate3dで指への追従性を最大化
 * - デザイン：V5.3の「白ベース・黒ヘッダー」を完全維持
 */
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const preventDefault = (e: TouchEvent) => {
      if (zoom > 1.05 || e.touches.length > 1) {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans overflow-hidden touch-none"
        ...
        <div className="p-8 pb-40">
          <main className="max-w-[95%] mx-auto grid lg:grid-cols-[1fr,400px] gap-8">
            <div className="space-y-6">
              ...
              <div className="flex justify-end gap-4"><button onClick={()=>setShots(shots.slice(0,-1))} className="bg-black text-white px-8 py-3 rounded-2xl font-black shadow-lg">Undo</button></div>
            </div>

            <aside className="space-y-6">
              <div className="bg-white border-2 border-gray-100 rounded-[2rem] p-6 h-[500px] overflow-y-auto">
                {isRangeMode ? <div className="text-sm whitespace-pre-wrap">{getAIAnalysis(stats.all)}</div> : shots.map((s, i) => (
                  <div key={s.id} className="flex gap-3 mb-4 border-b pb-4 items-center">
                    ...
                    <input value={s.comment} onChange={e=>{const n=[...shots]; n[i].comment=e.target.value; setShots(n);}} className="flex-1 text-sm outline-none pl-3 border-l" placeholder="備考..." />
                  </div>
                ))}
              </div>
              <textarea value={note} onChange={e=>setNote(e.target.value)} className="w-full bg-gray-50 border rounded-[2rem] p-6 h-32 outline-none" placeholder="メモ..." />
            </aside>
          </main>

          <section className="mt-20 border-t pt-10 px-4">
            <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest text-center mb-8">History</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {stats.filtered.map(h => (
                <button key={h.id} onClick={()=>loadHistory(h)} className={`p-6 rounded-[2rem] border-4 text-left ${editingId===h.id ? "bg-black text-white" : "bg-white border-gray-100"}`}>
                  <div className="text-xs opacity-60 mb-2">{h.date}</div><div className="font-black truncate uppercase">{h.place || "PRACTICE"}</div>
                </button>
              ))}
            </div>
          </section>
      ...
      <footer className="fixed bottom-0 left-0 w-full bg-black/90 text-white p-4 flex justify-around items-center z-50">
        <span className="text-[10px] font-mono text-gray-400 uppercase italic">V7.2 Final Fix</span>
